...
<aryawjan-project-guidelines>
# Project: Aryawjan Clinic AI (Viber + AI Dashboard)

## Context
A full-stack clinic management system that integrates Viber API for patient interaction and an AI-powered Dashboard for doctors and pharmacists.

## Tech Stack
- **Framework:** Laravel 12 (Modular Structure using InterNachi/Modular)
- **Local Development:** Laravel Sail
- **UI/Admin:** Filament PHP v5
- **AI:** Gemini 3.1 Flash-Lite (via Laravel AI SDK)
- **Database:** PostgreSQL with pgvector (for RAG)
- **Communication:** Viber API

## Modules Structure
1. **User:** Roles (Admin, Doctor, Pharmacist, Patient) via Spatie.
2. **Clinic:** - `DoctorSchedule` (Saves days as JSON array: ["Mon", "Tue"]).
   - `Appointment` (Token system).
3. **Pharmacy:** - `Medicine` (Inventory).
   - `PharmacyRequest` (AI-suggested meds for pharmacist, doctor approval).
4. **AI:** - RAG Logic (Medical books vector search).
   - Triage Service (Classifies symptoms into Path A: Doctor or Path B: Pharmacy).
5. **Viber:** - Webhook handling & outgoing messages.

## Business Logic Flows
- **Triage Flow:** Patient (Viber) -> AI Analysis -> (Serious -> Book Appointment) OR (Minor -> Pharmacist Approval -> Sell Meds).
- **RAG Flow:** Doctor/AI queries medical knowledge base stored in `medical_knowledge` table using vector embeddings.
</aryawjan-project-guidelines>
